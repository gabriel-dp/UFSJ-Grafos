...
class VertexGroup {
   ...
   public:
    ...
    void copy(VertexGroup vg) {
        vertexes.clear();
        for (Vertex v : vg.getVertexes()) {
            vertexes.insert(v);
        }
    }
    ...
    bool containsOne(VertexGroup group) {
        for (Vertex v : group.getVertexes()) {
...
class Graph {
   ...
   public:
    ...
    void addEdge(Vertex origin, Vertex destination, Length length = 1) {
        addVertex(origin);
    ...
    void print() {
        for (Vertex v : vertexes.getVertexes()) {
            std::cout << v << "\n";
        }
        for (Edge e : edges) {
            std::cout << e.getOrigin() << " -> " << e.getDestination() << " (" << e.getLength() << ")\n";
        }
    }
};
